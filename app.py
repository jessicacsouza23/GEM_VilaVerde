...
if menu == "🏠 Secretaria":
    ...
    # --- ABA 2: PLANEJAMENTO (COM REGRAS DE REPETIÇÃO E FIXAS) - V75 CORRIGIDA ---
with tab_plan:
    ...
    if sabados:
        ...
        calendario_db = db_get_calendario()
        df_historico_total = pd.DataFrame(db_get_historico()) 
        ...
        if data_sel_str not in calendario_db:
                ...
                if st.button("🚀 GERAR ESCALA AUTOMÁTICA", use_container_width=True, type="primary"):
                    ...
                    for h in [HORARIOS[1], HORARIOS[2], HORARIOS[3]]:
                        ...
                        for aluna in alunas_pratica_h:
                            ...
                            # SEGURANÇA NA SALA (V75) - Evita ValueError
                            if prof_final in PROFESSORAS_LISTA:
                                sala_num = (PROFESSORAS_LISTA.index(prof_final) % 7) + 1
                                mapa[aluna][h] = f"{nota} SALA {sala_num} | {prof_final}"
            ...
            df_raw = df_raw[["Aluna", "Turma"] + HORARIOS]
            df_editado = st.data_editor(df_raw, use_container_width=True, hide_index=True, key="editor_v75")
