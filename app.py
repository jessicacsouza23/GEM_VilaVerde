        ...
        with st.expander(f"📅 SÁBADO: {d_str}"):
            if d_str not in calendario_anual:
                ...
                if st.button(f"🚀 Gerar Rodízio para {d_str}", key=f"btn_{d_str}"):
                    escala_final = []
                    # LÓGICA DE SEQUÊNCIA: Após a Igreja, define quem vai para onde em cada horário
                    fluxo = {
                    ...
                    for t_nome, alunas in TURMAS.items():
                        for i, aluna in enumerate(alunas):
                            # Primeira aula: Sempre Igreja
                            agenda = {"Aluna": aluna, "Turma": t_nome, HORARIOS_LABELS[0]: "⛪ IGREJA"}
                            
                            # Sequência das próximas 3 aulas
                            for h_idx in [1, 2, 3]:
                                ...
                                if cfg["Teo"] == t_nome:
                                    ...
                                    # Lógica Prática: Pega professoras disponíveis (que não estão na Teoria/Solfejo ou Folga)
